...
export default function SearchBox({ onPick }: { onPick: (t: SearchTarget) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        {(["artist", "genre", "playlist"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`rounded-full px-3 py-1 border border-gray-600 ${tab === t ? "bg-gray-700 text-white" : "bg-gray-800 text-gray-200"}`}
          >
            {t}
          </button>
        ))}
      ...
      <ul className="divide-y divide-gray-700 rounded border border-gray-700 bg-gray-800">
        {tab === "artist" &&
          artistResults.map((r) => (
            <li
              key={r.id}
